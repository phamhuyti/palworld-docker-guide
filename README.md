...
| **[admin-tool.py](admin-tool.py)** | 🎛️ **Bảng điều khiển quản trị + PWA** (Python, không cần cài gói): xem người chơi/FPS/uptime, thông báo, save, kick/ban/unban, shutdown/stop (tự save trước), **đếm ngược spam thông báo trước khi restart/tắt**, RCON console. Có **đăng nhập + khóa chống dò mật khẩu + TLS** để dùng qua VPN từ điện thoại; cài được lên màn hình chính Android. Xem [mục dưới](#-quản-trị-từ-điện-thoại-admin-toolpy--pwa) |
